...
public class BookProvider extends ContentProvider {
    ...
    static {
        ...
        bookFull.setTables(
                AlexandriaContract.BookEntry.TABLE_NAME + " LEFT OUTER JOIN " + //NON-NLS
                        AlexandriaContract.AuthorEntry.TABLE_NAME + " USING (" + AlexandriaContract.BookEntry._ID + ")" + //NON-NLS
                        " LEFT OUTER JOIN " + AlexandriaContract.CategoryEntry.TABLE_NAME + " USING (" + AlexandriaContract.BookEntry._ID + ")"); //NON-NLS NON-NLS
    }
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case BOOK_FULLDETAIL:
                String[] bfd_projection = {
                        ...
                        "group_concat(DISTINCT " + AlexandriaContract.AuthorEntry.TABLE_NAME + "." + AlexandriaContract.AuthorEntry.AUTHOR + ") as " + AlexandriaContract.AuthorEntry.AUTHOR, //NON-NLS NON-NLS
                        "group_concat(DISTINCT " + AlexandriaContract.CategoryEntry.TABLE_NAME + "." + AlexandriaContract.CategoryEntry.CATEGORY + ") as " + AlexandriaContract.CategoryEntry.CATEGORY //NON-NLS NON-NLS
                };
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (match) {
            ...
            case AUTHOR: {
                ...
                if (_id > 0)
                    returnUri = AlexandriaContract.AuthorEntry.buildAuthorUri(values.getAsLong("_id")); //NON-NLS
                else
                    throw new android.database.SQLException("Failed to insert row into " + uri);
                break;
            }
            case CATEGORY: {
                ...
                if (_id > 0)
                    returnUri = AlexandriaContract.CategoryEntry.buildCategoryUri(values.getAsLong("_id")); //NON-NLS
                else
                    throw new android.database.SQLException("Failed to insert row into " + uri);
                break;
            }
